...
public class VoidChunkGenerator implements IChunkGenerator
{
    ...
    protected static final IBlockState STONE = VoidBlocks.VOIDSTONE.getDefaultState();
	...
	public VoidChunkGenerator(World par1World, long par2, boolean par4)
	{
		worldObj = par1World;
	...
	public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            for (int j = 0; j < 16; ++j)
            {
                genTerrainBlocks(this.worldObj, this.rand, primer, x * 16 + i, z * 16 + j, this.depthBuffer[j + i * 16]);
            }
        }
    }
    ...
    {
        int i = 50; //worldIn.getSeaLevel();
        IBlockState iblockstate = VoidBlocks.ASH.getDefaultState();
        IBlockState iblockstate1 = VoidBlocks.VOIDSTONE.getDefaultState();
        int j = -1;
            ...
            else
            {
                ...
                else if (iblockstate2.getBlock() == VoidBlocks.VOIDSTONE)
                {
                    if (j == -1)
                    {
                        ...
                        {
                            iblockstate = VoidBlocks.ASH.getDefaultState();
                            iblockstate1 = VoidBlocks.VOIDSTONE.getDefaultState();
                        }
                        ...
                        else if (j1 < i - 7 - k)
                        {
                            iblockstate = Blocks.AIR.getDefaultState();
                            iblockstate1 = STONE;
                            chunkPrimerIn.setBlockState(i1, j1, l, VoidBlocks.ASH.getDefaultState());
                        }
                        else
                        {
